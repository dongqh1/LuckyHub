...
abstract class AbstractSimulatorGateway<R extends GatewayRequest> {
 ...
 private Serialized serialize(R request){try{String payload=json.writeValueAsString(request);return new Serialized(payload,digest(payload));}catch(JacksonException e){throw new IllegalArgumentException("无法序列化模拟供应方请求",e);}}
